...
public class Clipboard {
    ...
    public Clipboard(final ControllerBuilder controllerBuilder) {
    ...
    public void open() {
        ...
        final Label defaultLabel = this.builderManager.getLabelBuilder().buildLabel("Clipboard | " + this.agentData.getUser() + "@" + this.agentData.getAddress(), 15, 20, Color.rgb(230, 230, 230), "Consolas", 17);
        final Line line = this.builderManager.getBasicLineBuilder().buildSimpleLine(Color.WHITE, 20, 54, 485, 54);

        final Text logText = this.builderManager.getTextBuilder().buildText("Loading content, please wait...", 3, 3, Color.WHITE, "Consolas", 12);

        final ScrollPane scrollPane = this.builderManager.getScrollPaneBuilder().buildScrollPane(true, logText, 15, 70, 475, 150);
        logText.wrappingWidthProperty().bind(this.scene.widthProperty());

        final Button copyButton = this.builderManager.getButtonBuilder().buildButton(null, "Copy Clipboard", 128, 245, Color.RED, "Verdana", 12, 250, 25, "agents-clipboard-copy");

        scrollPane.setHbarPolicy(ScrollPane.ScrollBarPolicy.NEVER);
        scrollPane.setVbarPolicy(ScrollPane.ScrollBarPolicy.NEVER);
        scrollPane.setId("agents-clipboard-scrollPane");
        ...
        copyButton.setOnAction(event -> {
            ...
            this.master.getNotification().notify("Clipboard", "Clipboard was successfully copied");
        });
    ...
    private void sendRequest() {
        ...
        this.waiting.add(id);
        this.master.getSocketUtils().write(this.agentData.getId(), "clipboard:" + id);
    }
